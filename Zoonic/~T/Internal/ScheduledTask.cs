...
namespace Zoonic.Concurrency
{
    ...
    abstract class ScheduledTask<T> : IScheduledRunnable
    {
        const int CancellationProhibited = 1;
        const int CancellationRequested = 1 << 1;

        protected readonly Zoonic.Concurrency.TaskCompletionSource Promise;
        protected readonly Executor<T> Executor;
